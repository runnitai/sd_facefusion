...
def run_ffmpeg(args: List[str]) -> bool:
    status = FFStatus()
    status.update("Extracting frames from video...")
    ...
    while process_manager.is_processing() and process.poll() is None:
        sleep(0.5)
    status.update("Extracting frames from video... Done in " + str(datetime.now() - start_time))
...
def extract_frames(target_path: str, video_resolution: str, video_fps: Fps) -> bool:
    ...
    resolution = video_resolution.replace('x', ':')
    if trim_frame_start is not None and trim_frame_end is not None:
        ...
        commands.extend(['-vf', 'trim=start_frame=' + str(trim_frame_start) + ',scale=' + resolution + ',fps=' + str(video_fps)])
    ...
    print(f"Extracting frames from video: '{' '.join(commands)}'")
...
def merge_video(target_path: str, video_fps: Fps) -> bool:
    ...
    print(f"Merging frames to video: '{' '.join(commands)}'")
    return run_ffmpeg_progress(commands)
...
def copy_image(target_path: str, output_path: str, image_resolution: str) -> bool:
    ...
    commands = ['-i', target_path, '-q:v', str(output_image_compression), '-vf', 'scale=' + str(image_resolution), '-y',
                output_path]
    return run_ffmpeg_progress(commands)
...
def restore_audio(target_path: str, output_path: str, video_fps: Fps) -> bool:
    ...
    print(f"Restoring audio to video: '{' '.join(commands)}'")
    return run_ffmpeg_progress(commands)
...
def replace_audio(target_path: str, audio_path: str, output_path: str) -> bool:
    ...
    print(f"Replacing audio in video: '{' '.join(commands)}'")
    return run_ffmpeg_progress(commands)
