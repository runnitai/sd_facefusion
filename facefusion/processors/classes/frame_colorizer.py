...
from facefusion.processors.optimizations.gpu_cv_ops import resize_gpu_or_cpu, cvt_color_gpu_or_cpu
...
class FrameColorizer(BaseProcessor):
    ...
    def prepare_temp_frame(self, temp_vision_frame: VisionFrame) -> VisionFrame:
        ...
        # Use GPU-accelerated color conversion if available
        temp_vision_frame = cvt_color_gpu_or_cpu(temp_vision_frame, cv2.COLOR_BGR2GRAY)
        temp_vision_frame = cvt_color_gpu_or_cpu(temp_vision_frame, cv2.COLOR_GRAY2RGB)

        if model_type == 'ddcolor':
            temp_vision_frame = (temp_vision_frame / 255.0).astype(numpy.float32)
            temp_vision_frame = cvt_color_gpu_or_cpu(temp_vision_frame, cv2.COLOR_RGB2LAB)[:, :, :1]
            ...
            temp_vision_frame = cvt_color_gpu_or_cpu(temp_vision_frame, cv2.COLOR_LAB2RGB)

        # Use GPU-accelerated resize if available
        temp_vision_frame = resize_gpu_or_cpu(temp_vision_frame, model_size)
    ...
    def merge_color_frame(self, temp_vision_frame: VisionFrame, color_vision_frame: VisionFrame) -> VisionFrame:
        ...
        # Use GPU-accelerated resize if available
        color_vision_frame = resize_gpu_or_cpu(color_vision_frame, (temp_vision_frame.shape[1], temp_vision_frame.shape[0]))

        if model_type == 'ddcolor':
            temp_vision_frame = (temp_vision_frame / 255.0).astype(numpy.float32)
            temp_vision_frame = cvt_color_gpu_or_cpu(temp_vision_frame, cv2.COLOR_BGR2LAB)[:, :, :1]
            color_vision_frame = numpy.concatenate((temp_vision_frame, color_vision_frame), axis=-1)
            color_vision_frame = cvt_color_gpu_or_cpu(color_vision_frame, cv2.COLOR_LAB2BGR)
        ...
        if model_type == 'deoldify':
            ...
            color_vision_frame = cvt_color_gpu_or_cpu(color_vision_frame, cv2.COLOR_BGR2RGB).astype(numpy.uint8)
            color_vision_frame = cvt_color_gpu_or_cpu(color_vision_frame, cv2.COLOR_BGR2LAB)
