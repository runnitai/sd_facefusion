...
def register_args(program: ArgumentParser) -> None:
    ...
    if group_processors:
        group_processors.add_argument('--face-debugger-items', help=wording.get('help.face_debugger_items').format(
            choices=', '.join(processors_choices.face_debugger_items)),
                                      default=config.get_str_list('processors.face_debugger_items',
                                                                  'face-landmark-5/68 face-mask'),
                                      choices=processors_choices.face_debugger_items, nargs='+',
...
def debug_face(target_face: Face, temp_vision_frame: VisionFrame) -> VisionFrame:
    ...
    has_face_landmark_5_fallback = numpy.array_equal(target_face.landmark_set.get('5'),
                                                     target_face.landmark_set.get('5/68'))
    has_face_landmark_68_fallback = numpy.array_equal(target_face.landmark_set.get('68'),
                                                      target_face.landmark_set.get('68/5'))
    ...
    if 'face-mask' in face_debugger_items:
        ...
        cv2.drawContours(temp_vision_frame, inverse_contours, -1,
                         tertiary_color if has_face_landmark_5_fallback else secondary_color, 2)
    ...
    if 'face-landmark-5/68' in face_debugger_items and numpy.any(target_face.landmark_set.get('5/68')):
        ...
        for index in range(face_landmark_5_68.shape[0]):
            cv2.circle(temp_vision_frame, (face_landmark_5_68[index][0], face_landmark_5_68[index][1]), 3,
                       tertiary_color if has_face_landmark_5_fallback else secondary_color, -1)
    ...
    if 'face-landmark-68' in face_debugger_items and numpy.any(target_face.landmark_set.get('68')):
        ...
        for index in range(face_landmark_68.shape[0]):
            cv2.circle(temp_vision_frame, (face_landmark_68[index][0], face_landmark_68[index][1]), 3,
                       tertiary_color if has_face_landmark_68_fallback else secondary_color, -1)
    ...
    if 'face-landmark-68/5' in face_debugger_items and numpy.any(target_face.landmark_set.get('68')):
        ...
        for index in range(face_landmark_68.shape[0]):
            cv2.circle(temp_vision_frame, (face_landmark_68[index][0], face_landmark_68[index][1]), 3, tertiary_color,
                       -1)
    ...
    if bounding_box[3] - bounding_box[1] > 50 and bounding_box[2] - bounding_box[0] > 50:
        ...
        if 'face-detector-score' in face_debugger_items:
            ...
            top = top + 20
            cv2.putText(temp_vision_frame, face_score_text, (left, top), cv2.FONT_HERSHEY_SIMPLEX, 0.5, primary_color,
                        2)

        if 'face-landmarker-score' in face_debugger_items:
            face_score_text = str(round(target_face.score_set.get('landmarker'), 2))
            top = top + 20
            cv2.putText(temp_vision_frame, face_score_text, (left, top), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                        tertiary_color if has_face_landmark_5_fallback else secondary_color, 2)
        ...
        if 'gender' in face_debugger_items:
            ...
            top = top + 20
            cv2.putText(temp_vision_frame, face_gender_text, (left, top), cv2.FONT_HERSHEY_SIMPLEX, 0.5, primary_color,
                        2)
