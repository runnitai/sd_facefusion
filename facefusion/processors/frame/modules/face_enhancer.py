...
def register_args(program: ArgumentParser) -> None:
    program.add_argument('--face-enhancer-model', help=wording.get('frame_processor_model_help'), default='gfpgan_1.4',
                         choices=frame_processors_choices.face_enhancer_models)
    program.add_argument('--face-enhancer-blend', help=wording.get('frame_processor_blend_help'), type=int, default=80,
...
def pre_process(mode: ProcessMode, job: JobParams) -> bool:
    ...
    if not is_download_done(model_url, model_path):
        ...
        return False
    if mode in ['output', 'preview'] and not is_image(job.target_path) and not is_video(job.target_path):
...
def enhance_face(target_face: Face, temp_frame: Frame) -> Frame:
    if JOB:
        face_mask_blur = JOB.face_mask_blur
        face_mask_padding = JOB.face_mask_padding
    else:
...
def process_video(source_paths: str, temp_frame_paths: List[str], status) -> None:
