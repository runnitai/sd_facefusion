...
from facefusion.musetalk.utils.face_parsing.model import BiSeNet
...
class FaceParsing():
    def __init__(self, left_cheek_width=80, right_cheek_width=80):
        ...
        self.cheek_mask = self._create_cheek_mask(left_cheek_width=left_cheek_width,
    ...
    def model_init(self,
        ...
        if resnet_path is None:
            resnet_path = resolve_relative_path(
                '../.assets/models/musetalk_v15/face-parse-bisent/resnet18-5c106cde.pth')
        if model_pth is None:
            model_pth = resolve_relative_path('../.assets/models/musetalk_v15/face-parse-bisent/79999_iter.pth')
        logger.info(f"Loading face parsing model from {model_pth}")
        logger.info(f"Loading resnet weights from {resnet_path}")
        net = BiSeNet(resnet_path)
        if torch.cuda.is_available():
            logger.info("Using CUDA for face parsing model.")
            net.cuda()
            net.load_state_dict(unsafe_torch_load(model_pth, weights_only=False))
        else:
            logger.info("Using CPU for face parsing model.")
            net.load_state_dict(unsafe_torch_load(model_pth, map_location=torch.device('cpu'), weights_only=False))
        net.eval()
        return net
