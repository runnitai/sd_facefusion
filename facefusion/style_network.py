...
class KernelFilter(nn.Module):
    ...
    def apply_filter(input_, filter_):
        ...
        results = []
        for inp, fil in zip(input_chunk, filter_chunt):
...
class InstanceNorm(nn.Module):
    def __init__(self, epsilon=1e-8, style_num=1):
        ...
        self.x_max = [None for _ in range(self.style_num)]
        self.x_min = [None for _ in range(self.style_num)]
...
class Decoder(nn.Module):
    def __init__(self, style_num=1):
        ...
        for layer in range(1, 5):
            self.style_means.append([None for _ in range(style_num)])
            self.style_stds.append([None for _ in range(style_num)])
...
class TransformerNet(nn.Module):
    def __init__(self, style_num=1):
        ...
        # We collect multiple content patches, then compute norms all at once
        self.F_patches = []
        # We store each style's features in a list:
        self.F_style = [None for _ in range(style_num)]
        self.style_num = style_num
...
class Stylization:
    def __init__(self, checkpoint, cuda=False, style_num=1):
        if cuda:
        ...
        self.model = TransformerNet(style_num=style_num).to(self.device)
        self.style_num = style_num
    ...
    def add(self, patch):
        with torch.no_grad():
            # Turn the patch into a proper tensor
            patch = numpy2tensor(patch).to(self.device)
            patch = transform_image(patch)

            # Now encode it into a 512-channel feature
            patch_features = self.model.generate_content_features(patch)

            # Finally add those features to the model
            self.model.add_patch(patch_features)
        torch.cuda.empty_cache()

    def compute(self):
        with torch.no_grad():
            self.model.compute_norm()
        torch.cuda.empty_cache()
    ...
    # -------------------------------------------------
    # For passing in multiple styles at once:
    # -------------------------------------------------
    def prepare_styles(self, style_list: List[np.ndarray]):
        """
        Prepares (encodes) multiple style images at once.
        By default, they will be averaged equally if style_weight is not specified
        when calling `transfer` or `transfer_with_weights`.
        """
        if len(style_list) != self.style_num:
            raise ValueError(
                f"Expected {self.style_num} style images, but got {len(style_list)}."
            )
        with torch.no_grad():
            for i, style in enumerate(style_list):
                style_tensor = numpy2tensor(style).to(self.device)
                style_tensor = transform_image(style_tensor)
                self.model.generate_style_features(style_tensor, style_id=i)
        torch.cuda.empty_cache()

    def prepare_style(self, style):
        """
        (Original single-style approach.)
        If you only have 1 style, you can use this function.
        """
        self.prepare_styles([style])
    ...
    def transfer_with_weights(self, frame, style_weight: List[float]):
        """
        Transfer with explicit style weights for each style.
        The `style_weight` list must match self.style_num in length.
        """
        with torch.no_grad():
            ...
            # Stylization
            frame = self.model(frame, style_weight=style_weight)
    ...
    #     """
    #     Transfer by equally averaging all the styles (or just 1 style if style_num=1).
    #     """
    #     # If multiple styles, we default to an equal weight sum = 1.0
    #     with torch.no_grad():
    #         frame = numpy2tensor(frame).to(self.device)
    #         frame = transform_image(frame)
    #
    #         if self.style_num > 1:
    #             style_weight = [1.0 / self.style_num] * self.style_num
    #         else:
    #             style_weight = [1.0]  # single style
    #
    #         frame = self.model(frame, style_weight=style_weight)
    #         frame_result = transform_back_image(frame)
    #         frame_result = tensor2numpy(frame_result)
    ...
    def transfer(self, frame):
        with torch.no_grad():
            # Convert raw frame to a tensor on the correct device
            frame = numpy2tensor(frame).to(self.device)
            frame = transform_image(frame)

            # Encode the 3-channel image into 512-channel features:
            content_feat = self.model.generate_content_features(frame)

            # Prepare style weights if you have multiple styles:
            if self.style_num > 1:
                style_weight = [1.0 / self.style_num] * self.style_num
            else:
                style_weight = [1.0]

            # Now pass the 512-channel features into your model (NOT the raw frame):
            out = self.model(content_feat, style_weight=style_weight)

            # Convert back to an OpenCV-style image for display
            out = transform_back_image(out)
