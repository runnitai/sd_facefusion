...
def warp_face_by_kps(temp_frame: Frame, kps: Kps, template: Template, crop_size: Size) -> Tuple[Frame, Matrix]:
    ...
    crop_frame = cv2.warpAffine(temp_frame, affine_matrix, crop_size, borderMode=cv2.BORDER_REPLICATE,
...
def paste_back(temp_frame: Frame, crop_frame: Frame, crop_mask: Mask, affine_matrix: Matrix) -> Frame:
    ...
    paste_frame[:, :, 0] = inverse_crop_mask * inverse_crop_frame[:, :, 0] + (1 - inverse_crop_mask) * temp_frame[:, :,
                                                                                                       0]
    paste_frame[:, :, 1] = inverse_crop_mask * inverse_crop_frame[:, :, 1] + (1 - inverse_crop_mask) * temp_frame[:, :,
                                                                                                       1]
    paste_frame[:, :, 2] = inverse_crop_mask * inverse_crop_frame[:, :, 2] + (1 - inverse_crop_mask) * temp_frame[:, :,
                                                                                                       2]
    return paste_frame
