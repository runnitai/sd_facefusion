...
def main():
    ...
    video_frames = './inputs/ambush_4/*.png'
    ...
    for res_dir in [result_frames_path, result_videos_path]:
    ...
    os.makedirs('{}/{}'.format(result_frames_path, name), exist_ok=True)
    ...
    # Build model
    transfer_model = Stylization(checkpoint_path, cuda)
    transfer_model.prepare_style(style)
    ...
    transfer_model.prepare_global(frame_list)
    ...
    for i in range(frame_num):
        ...
        new_input_frame = reshape_tool.process(input_frame)

        # Stylization
        styled_input_frame = transfer_model.transfer(new_input_frame)
