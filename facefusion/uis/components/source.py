...
def remote_update(files1, files2) -> Tuple[
    ...
    """Handle updates triggered by processor changes or other external events - preserve source files"""
    
    # Add debug logging
    logger.info(f"remote_update called with files1: {files1 is not None}, files2: {files2 is not None}", __name__)
    
    # ALWAYS preserve existing source files unless explicitly cleared by user
    # Get current source paths from state manager (these are the actual source files)
    ...
    # Just refresh UI with current state without changing the actual source files
    ...
    # Show both audio rows if either has audio
    image_row_visibility = has_image_1 or has_image_2
    
    logger.info(f"Preserved all source files - audio1: {audio_path_1}, image1: {image_path_1}, audio2: {audio_path_2}, image2: {image_path_2}", __name__)

    return source_audio_1, source_image_1, source_audio_2, source_image_2, audio_row_1, gradio.update(visible=image_row_visibility)
